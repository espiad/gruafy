...
function copyFor(state: OrderState, role: 'cliente' | 'gruero', etaMin?: number | null): Copy | null {
  ...
  const client: Partial<Record<OrderState, Copy>> = {
    ...
    provider_en_route: { emoji: '🚚', title: 'Tu grúa va en camino', sub: `${eta}Seguila en vivo en el mapa.`, tone: 'success', pulse: true },
    ...
    in_transit: { emoji: '🛣️', title: 'En camino al destino', sub: 'Seguí el recorrido en vivo.', tone: 'success', pulse: true },
  ...
  const provider: Partial<Record<OrderState, Copy>> = {
    ...
    provider_en_route: { emoji: '🚚', title: 'Vas en camino a la recogida', sub: 'Compartí tu ubicación y avisá cuando llegues.', tone: 'success' },
    ...
    in_transit: { emoji: '🛣️', title: 'En camino al destino', sub: 'Confirmá la entrega al llegar.', tone: 'green' },
